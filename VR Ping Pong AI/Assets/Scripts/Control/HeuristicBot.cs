...
public class HeuristicBot : CatcherBot
{
    ...
    private void OnEvent_racketHitBall(GameObject racketThatHitTheBall)
    {
        if (racketThatHitTheBall == gameObject)
        {
            ...
            reactionTimeDuration = Mathf.Max(0f,
                OurMath.sampleNormalDistribution(bot.heuristics.reactionTime_mean, bot.heuristics.reactionTime_stDev));
        }
    }
    ...
    protected override void Update()
    {
        if (reactionTimeTimer > 0f)
        {
            ...
            if (reactionTimeTimer > reactionTimeDuration)
            {
                ...
                Debug.Log("<HEURISTIC_BOT>: I am allowed to react now! reactionTimeDuration was " +
    ...
    protected override void hit()
    {
        ...
        if (tooHigh)
        {
            Debug.Log("<HEURISTIC_BOT>: I will shoot too high! My error rate for that was " +
                      bot.heuristics.errorRate_tooHigh.ToString());
            aimPos *= 2;
            aimHeight *= 2;
        ...
        if (tooLow)
        {
            Debug.Log("<HEURISTIC_BOT>: I will shoot too low! My error rate for that was " +
                      bot.heuristics.errorRate_tooLow.ToString());
            aimPos.z *= 0.1f;
            aimHeight /= 2;
            HitAndSaveShot(aimPos, aimHeight);
            return;
        ...
        {
            Debug.Log("<HEURISTIC_BOT>: I will replay a winning shot now! I have that many winning shots stored: " +
                      bot.heuristics.winningShots.Count.ToString() + ". That should be less or equal to " +
                      Statics.bot_winningPlays_listLength().ToString());
        ...
        {
            Debug.Log(
                "<HEURISTIC_BOT>: I will shoot to the left half of the table! My probability for that override was " +
                bot.heuristics.GetLeftShotOverrideProbability().ToString());
            aimPos.x = (invertXZ ? 1 : -1) * Mathf.Abs(aimPos.x);
        }
        if (goRight)
        {
            Debug.Log(
                "<HEURISTIC_BOT>: I will shoot to the right half of the table! My probability for that override was " +
                bot.heuristics.GetRightShotOverrideProbability().ToString());
            aimPos.x = (invertXZ ? -1 : 1) * Mathf.Abs(aimPos.x);
        }
        ...
        if (r2 < bot.heuristics.GetLowShotOverrideProbability())
            goLow = true;
        else if (r2 < bot.heuristics.GetLowShotOverrideProbability() +
                 bot.heuristics.GetMediumShotOverrideProbability())
            goMedium = true;
        else if (r2 < bot.heuristics.GetLowShotOverrideProbability() +
                 bot.heuristics.GetMediumShotOverrideProbability() + bot.heuristics.GetHighShotOverrideProbability())
            goHigh = true;

        if (goLow)
        {
            Debug.Log("<HEURISTIC_BOT>: I will shoot a low ball now! My probability for that override was " +
                      bot.heuristics.GetLowShotOverrideProbability().ToString());
            aimHeight = Random.Range(lowHeight - 0.2f, lowHeight + 1.0f);
        }
        if (goMedium)
        {
            Debug.Log("<HEURISTIC_BOT>: I will shoot a medium-height ball now! My probability for that override was " +
                      bot.heuristics.GetMediumShotOverrideProbability().ToString());
            aimHeight = Random.Range(medHeight - 1.0f, medHeight + 1.0f);
        }
        if (goHigh)
        {
            Debug.Log("<HEURISTIC_BOT>: I will shoot a high ball now! My probability for that override was " +
                      bot.heuristics.GetHighShotOverrideProbability().ToString());
