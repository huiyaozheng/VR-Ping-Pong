...
/// <summary>
/// Handles the collision between the huamn player's racket and the ball.
/// </summary>
public class PlayerBallCollisions : MonoBehaviour
{
    ...
    public float normalInfluence = 0.05f
        ; // We multiply the (unit) normal vector by the length of the racket velocity, and then by this number!
    ...
    void OnCollisionEnter(Collision col)
    {
        ...
        Vector3 racketVelocity =
            ...
            (float) positionTrackingDepthInFrames; // normalize by dividing by the number of frames in between
        ...
        if (col.gameObject == ball)
        {
            ...
            ) // We want the normal to face the same way as racket velocity, rather than opposite
                racketNormal *= -1;
